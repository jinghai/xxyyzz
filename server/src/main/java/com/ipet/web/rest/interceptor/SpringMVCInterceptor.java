 ...
 * （一）实现HandlerInterceptor接口 HandlerInterceptor
 ...
 * 的时候就会是调用当前请求的Controller 方法。 （2 ）postHandle (HttpServletRequest request,
 ...
 * 处理之后的ModelAndView 对象进行操作。postHandle 方法被调用的方向跟preHandle
 * 是相反的，也就是说先声明的Interceptor 的postHandle 方法反而会后执行，这和Struts2 里面的Interceptor
 * 的执行过程有点类型。Struts2 里面的Interceptor 的执行过程也是链式的，只是在Struts2
